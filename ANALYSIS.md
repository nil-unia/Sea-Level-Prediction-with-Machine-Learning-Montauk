...
Sea level rise is one of the more measurable, data-rich consequences of climate change, which makes it a strong candidate for applying time series machine learning: the signal is real, the historical record is long (NOAA tide gauge data goes back over a century at many stations), and the results are directly relevant to public infrastructure and community planning.
...
**Model:** Facebook Prophet, fit with default parameters (additive trend + yearly seasonality; weekly/daily seasonality auto-disabled since the data is monthly).
...
|---|---|---|
| MAE | **0.041 m** (~4 cm) | On a series where full-record variation spans roughly ±0.2 m, this is a solid fit — the model tracks the trend closely |
...
- It smooths over — and therefore misses — short-term anomalies (storm surge events, seasonal current shifts), which is expected: Prophet is a trend/seasonality decomposition, not a physical ocean model.
...
**Comparison to NOAA regional scenarios:** NOAA's "Regional Scenarios" tool for this station provides multiple climate-model-informed projections (low to high emissions pathways) through 2100. The Prophet model's linear-ish extrapolation of the historical rate tends to track closest to NOAA's **intermediate-low to intermediate** scenarios, since it has no way to account for potential *acceleration* in sea level rise from ice sheet dynamics — it can only extend the rate of change already present in the historical record.
...
- **What it isn't:** a substitute for physically-based climate models. Prophet cannot represent ice sheet melt dynamics, thermal expansion, or land subsidence — it can only project forward whatever pattern already existed in the training window.
...
*[This section is a starting scaffold — fill in the specifics of your BNL work so it reads as your own experience rather than a generic description.]*

Working on this project drew directly on research practices developed during my time at Brookhaven National Laboratory, including:
...
- **Reproducibility** — building the pipeline as a documented, re-runnable notebook (rather than one-off analysis) so someone else — a classmate, a community member, a local planning board — can plug in their own station's data and get a trustworthy result without needing to understand the underlying statistics.
- **[Add a specific technique, tool, or dataset-handling practice from your BNL research that you reused or adapted here.]**

The intent behind open-sourcing this project is to make that same rigor available to non-specialists: **local communities, students, or civic groups** who want a data-backed starting point for thinking about sea level rise in their area, without needing access to a national lab's modeling infrastructure. Anyone can fork this repo, swap in their own NOAA station, and get a station-specific forecast with honest error bars in under an hour.
...
- Test the pipeline against 2–3 additional NOAA stations with different regional characteristics (e.g., a Gulf Coast station experiencing land subsidence vs. a West Coast station) to see how portable the default Prophet configuration is.
